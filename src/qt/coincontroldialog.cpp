...
void CoinControlDialog::updateLabels()
{
    ...
    ui->labelTitle->setText(fSelectTransparent ?
            "Select SBX Outputs to Spend" :
            "Select Shielded SBX to Spend");
    ...
    int nDisplayUnit = BitcoinUnits::SBX;
                                          ...
                                          const bool isChange)
{
    ...
    // if listMode or change => show SBX address. In tree mode, address is not shown again for direct wallet address outputs
    if (!treeMode) {
