...
#include "qt/safebox/forms/ui_addresseswidget.h"
#include "qt/safebox/addresslabelrow.h"
#include "qt/safebox/addnewaddressdialog.h"
#include "qt/safebox/tooltipmenu.h"
...
AddressesWidget::AddressesWidget(SBXGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        auto sbxAdd = Standard::DecodeDestination(address.toUtf8().constData(), isStakingAddress);

        if (!Standard::IsValidDestination(sbxAdd)) {
            setCssEditLine(ui->lineEditAddress, false, true);
            inform(tr("Invalid Contact Address"));
            return;
        }

        if (walletModel->isMine(sbxAdd)) {
            setCssEditLine(ui->lineEditAddress, false, true);
        ...
        bool isShielded = walletModel->IsShieldedDestination(sbxAdd);
        if (walletModel->updateAddressBookLabels(sbxAdd, label.toUtf8().constData(),
