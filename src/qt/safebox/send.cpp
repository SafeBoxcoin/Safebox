...
#include "qt/safebox/send.h"
#include "qt/safebox/forms/ui_send.h"
#include "qt/safebox/addnewcontactdialog.h"
#include "qt/safebox/qtutils.h"
#include "qt/safebox/sendchangeaddressdialog.h"
#include "qt/safebox/optionbutton.h"
#include "qt/safebox/sendconfirmdialog.h"
#include "qt/safebox/guitransactionsutils.h"
#include "qt/safebox/loadingdialog.h"
...
SendWidget::SendWidget(SBXGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    coinIcon(new QPushButton())
{
    ...
    setCssProperty(coinIcon, "coin-icon-safebox");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onSBXSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onSBXSelected(false);});
...
void SendWidget::showHideCheckBoxDelegations(CAmount delegationBalance)
{
    ...
    // coincontrol is not selected, and we are trying to spend transparent SBXs.
...
void SendWidget::onShieldCoinsClicked()
{
    ...
    if (availableBalance > 0) {
        ...
        ProcessSend(recipients, true, [this](QList<SendCoinsRecipient>& recipients) {
            ...
            if (!res.result) {
                inform(tr("Error generating address to shield SBXs"));
                return false;
            }
        ...
        inform(tr("You don't have any transparent SBXs to shield."));
    }
}
...
void SendWidget::onSBXSelected(bool _isTransparent)
{
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        auto safeboxAdd = Standard::DecodeDestination(address.toStdString(), isStakingAddr);

        if (!Standard::IsValidDestination(safeboxAdd) || isStakingAddr) {
            inform(tr("Invalid address"));
            return;
        }

        if (walletModel->isMine(safeboxAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(safeboxAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
