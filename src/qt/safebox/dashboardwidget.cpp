...
#include "qt/safebox/dashboardwidget.h"
#include "qt/safebox/forms/ui_dashboardwidget.h"
#include "qt/safebox/sendconfirmdialog.h"
#include "qt/safebox/txrow.h"
...
DashboardWidget::DashboardWidget(SBXGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareSbx, "square-chart-sbx");
    //setCssProperty(ui->labelSquarezSbx, "square-chart-zsbx");
    setCssProperty(ui->labelSbx, "text-chart-sbx");
    //setCssProperty(ui->labelZsbx, "text-chart-zsbx");
    ...
    setCssProperty(ui->labelAmountSbx, "text-stake-sbx-disable");
    //setCssProperty(ui->labelAmountZsbx, "text-stake-zsbx-disable");
    ui->labelAmountMNRewards->setText("0 SBX");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &SBXGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("SBX")
    updateDisplayUnit();
}
...
const QMap<int, QMap<QString, qint64>> DashboardWidget::getAmountBy()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        //bool isSbx = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZSBX;
        bool isSbx = modelIndex.data(TransactionTableModel::TypeRole).toInt() == TransactionRecord::StakeMint;
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair SBX, zSBX
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            //sbx = (pair.first != 0) ? pair.first / 100000000 : 0;
            //zsbx = (pair.second != 0) ? pair.second / 100000000 : 0;
            //chartData->totalSbx += pair.first;
            //chartData->totalZsbx += pair.second;
            QMap<QString, qint64> pair = chartData->amountsByCache[num];
            sbx = (pair["sbx"] != 0) ? pair["sbx"] / 100000000 : 0;
            //zsbx = (pair["zsbx"] != 0) ? pair["zsbx"] / 100000000 : 0;
            mnrewards = (pair["mn"] != 0) ? pair["mn"] / 100000000 : 0;
            chartData->totalSbx += pair["sbx"];
            //chartData->totalZsbx += pair["totalZsbx"];
            chartData->totalMNRewards += pair["mn"];
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesSbx.append(sbx);    
        chartData->valuesMNRewards.append(mnrewards); 
        //chartData->valueszSbx.append(zsbx);

        int max = std::max(sbx, mnrewards);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->append(chartData->valuesSbx);
    //set1->append(chartData->valueszSbx);
    set1->append(chartData->valuesMNRewards);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    //if (chartData->totalSbx > 0 || chartData->totalZsbx > 0) {
    if (chartData->totalSbx > 0 || chartData->totalMNRewards > 0) {
        setCssProperty(ui->labelAmountSbx, "text-stake-sbx");
        //setCssProperty(ui->labelAmountZsbx, "text-stake-zsbx");
    ...
    //forceUpdateStyle({ui->labelAmountSbx, ui->labelAmountZsbx});
    forceUpdateStyle({ui->labelAmountSbx, ui->labelAmountMNRewards});
    ui->labelAmountSbx->setText(GUIUtil::formatBalance(chartData->totalSbx, nDisplayUnit));
    //ui->labelAmountZsbx->setText(GUIUtil::formatBalance(chartData->totalZsbx, nDisplayUnit, true));
