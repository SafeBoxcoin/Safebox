...
#include "qt/safebox/forms/ui_snackbar.h"
#include "qt/safebox/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(SBXGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &SBXGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
