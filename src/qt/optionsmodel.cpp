...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::SBX);
    ...
    if (settings.contains("nAnonymizeSbxAmount") || reset)
        gArgs.SoftSetArg("-anonymizesbxamount", settings.value("nAnonymizeSbxAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in sbx.cpp during shotdown to also remove the window positions
