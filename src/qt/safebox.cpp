...
#include "config/safebox-config.h"
#endif
...
#include "qt/safebox/splash.h"
#include "qt/safebox/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("safebox-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in safebox.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in safebox.qrc)
...
/** Class encapsulating SafeBox Core startup and shutdown.
...
public:
    ...
    /// Get window identifier of QMainWindow (SBXGUI)
...
{
    window = new SBXGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &SBXGUI::detectShutdown);
}
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &SBXGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(SBXGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(SBXGUI::DEFAULT_WALLET);
        ...
        // SBX: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &SBXGUI::handlePaymentRequest);
        connect(window, &SBXGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. SBX can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(safebox_locale);
    Q_INIT_RESOURCE(safebox);
    ...
    /// 6. Determine availability of data directory and parse safebox.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("SafeBox Core"),
    ...
    try {
        ...
        QMessageBox::critical(0, QObject::tr("SafeBox Core"), QObject::tr("Error: %1").arg(e.what()));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // safebox: links repeatedly have their payment requests routed to this process:
