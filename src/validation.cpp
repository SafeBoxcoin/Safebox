...
#include "zsafebox/zerocoin.h"
#include "zsafebox/zsafeboxmodule.h"
...
#if defined(NDEBUG)
#error "SBX cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in usafebox) are considered zero fee (for relaying, mining and transaction creation)
...
int64_t GetBlockValue(int nHeight)
{
	...
	else if (nHeight <= 14 && nHeight > 1) {
        nSubsidy = 1000000 * COIN;
    }
    //else if (nHeight <= 200 && nHeight > 14) {
    //    nSubsidy = 1 * COIN;
    //}
	else if (nHeight <= 50000 && nHeight > 14) {
        nSubsidy = 1 * COIN;
    }
	...
	else if (nHeight <= 175000 && nHeight > 125000) {
        nSubsidy = 130 * COIN;
    }
	else if (nHeight <= 225000 && nHeight > 175000) {
        nSubsidy = 110 * COIN;
    }
	...
	else if (nHeight <= 550000 && nHeight > 525000) {
        nSubsidy = 300 * COIN;
    }
	...
	else if (nHeight <= 950000 && nHeight > 800000) {
        nSubsidy = 110 * COIN;
    }
	else if (nHeight <= 1100000 && nHeight > 950000) {
        nSubsidy = 90 * COIN;
    }
	else if (nHeight <= 1250000 && nHeight > 1100000) {
        nSubsidy = 70 * COIN;
    }
	else if (nHeight <= 1400000 && nHeight > 1250000) {
        nSubsidy = 50 * COIN;
    }
	else if (nHeight <= 1425000 && nHeight > 1400000) {
        nSubsidy = 300 * COIN;
    }
	else if (nHeight <= 1575000 && nHeight > 1425000) {
        nSubsidy = 150 * COIN;
    }
	else if (nHeight <= 1725000 && nHeight > 1575000) {
        nSubsidy = 130 * COIN;
    }
	else if (nHeight <= 1875000 && nHeight > 1725000) {
        nSubsidy = 110 * COIN;
    }
	else if (nHeight <= 2025000 && nHeight > 1875000) {
        nSubsidy = 90 * COIN;
    }
	else if (nHeight <= 2175000 && nHeight > 2025000) {
        nSubsidy = 70 * COIN;
    }
	else if (nHeight <= 2325000 && nHeight > 2175000) {
        nSubsidy = 50 * COIN;
    }
	else if (nHeight <= 2350000 && nHeight > 2325000) {
        nSubsidy = 300 * COIN;
    }
	else if (nHeight <= 2500000 && nHeight > 2350000) {
        nSubsidy = 150 * COIN;
    }
	else if (nHeight <= 2650000 && nHeight > 2500000) {
        nSubsidy = 130 * COIN;
    }
	else if (nHeight <= 2800000 && nHeight > 2650000) {
        nSubsidy = 110 * COIN;
    }
	else if (nHeight <= 2950000 && nHeight > 2800000) {
        nSubsidy = 90 * COIN;
    }
	else if (nHeight <= 3100000 && nHeight > 2950000) {
        nSubsidy = 70 * COIN;
    }
	else if (nHeight <= 3125000 && nHeight > 3100000) {
        nSubsidy = 300 * COIN;
    }
    else if (nHeight <= 3275000 && nHeight > 3125000) {
        nSubsidy = 150 * COIN;
    }
    else if (nHeight <= 3425000 && nHeight > 3275000) {
        nSubsidy = 130 * COIN;
    }
    else if (nHeight <= 3575000 && nHeight > 3425000) {
        nSubsidy = 110 * COIN;
    }
    else if (nHeight <= 3725000 && nHeight > 3575000) {
        nSubsidy = 90 * COIN;
    }
    else if (nHeight <= 3875000 && nHeight > 3725000) {
        nSubsidy = 70 * COIN;
    }
	else if (nHeight > 3875000) {
        nSubsidy = 0 * COIN;
    }
  else {
      nSubsidy = 0 * COIN;
...
{
    util::ThreadRename("sbx-scriptch");
        ...
        if (tx.HasZerocoinSpendInputs()) {
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (isPublicSpend) {
                    ...
                    if (!ZSBXModule::ParseZerocoinPublicSpend(txIn, tx, state, publicSpend)){
                        return false;
                    }
...
bool CheckWork(const CBlock& block, const CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        // Sbx Specific reference to the block with the wrong threshold was used.
        const Consensus::Params& consensus = Params().GetConsensus();
        if ((block.nTime == (uint32_t) consensus.nSbxBadBlockTime) &&
                (block.nBits == (uint32_t) consensus.nSbxBadBlockBits)) {
            // accept SBX block minted with incorrect proof of work threshold
            return true;
        }
...
bool AcceptBlock(const CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> sbxInputs;
        std::vector<CTxIn> zSBXInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
            if(stakeIn.IsZerocoinSpend()){
                zSBXInputs.push_back(stakeIn);
            }else{
                sbxInputs.push_back(stakeIn);
            }
        }
        const bool hasSBXInputs = !sbxInputs.empty();
        const bool hasZSBXInputs = !zSBXInputs.empty();
        ...
        for (const auto& txIn : block.vtx) {
            ...
            for (const CTxIn& in: tx.vin) {
                if(consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC)) {
                    ...
                    if (isPrivZerocoinSpend || isPublicSpend) {
                        ...
                        if (isPublicSpend) {
                            libzerocoin::ZerocoinParams* params = consensus.Zerocoin_Params(false);
                            PublicCoinSpend publicSpend(params);
                            if (!ZSBXModule::ParseZerocoinPublicSpend(in, tx, state, publicSpend)){
                                return false;
                            }
                            spend = publicSpend;
                        } else {
                ...
                if(hasSBXInputs) {
                    ...
                    for (const CTxIn& sbxIn : sbxInputs)
                        if(sbxIn.prevout == in.prevout)
        ...
        if (isBlockFromFork) {
            ...
            // Now that this loop if completed. Check if we have zSBX inputs.
            if(hasZSBXInputs) {
                for (const CTxIn& zSbxInput : zSBXInputs) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(zSbxInput);
                    ...
                    if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, &spend, pindex->nHeight, UINT256_ZERO))
                        return state.DoS(100,error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                                   stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zsbx");
