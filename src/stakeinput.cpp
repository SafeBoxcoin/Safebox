...
CSbxStake* CSbxStake::NewSbxStake(const CTxIn& txin)
{
    if (txin.IsZerocoinSpend()) {
        error("%s: unable to initialize CSbxStake from zerocoin spend", __func__);
        return nullptr;
    }
    ...
    return new CSbxStake(txPrev->vout[txin.prevout.n],
                         txin.prevout,
                         pindexFrom);
}

bool CSbxStake::GetTxOutFrom(CTxOut& out) const
{
    out = outputFrom;
    return true;
}

bool CSbxStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(outpointFrom.hash, outpointFrom.n);
    return true;
}

CAmount CSbxStake::GetValue() const
{
    return outputFrom.nValue;
}

bool CSbxStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal, const bool onlyP2PK)
{
...
CDataStream CSbxStake::GetUniqueness() const
{
    //The unique identifier for a SBX stake is the outpoint
...
const CBlockIndex* CSbxStake::GetIndexFrom() const
{
    // Sanity check, pindexFrom is set on the constructor.
    if (!pindexFrom) throw std::runtime_error("CSbxStake: uninitialized pindexFrom");
    return pindexFrom;
}

// Verify stake contextual checks
bool CSbxStake::ContextCheck(int nHeight, uint32_t nTime)
{
