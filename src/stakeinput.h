...
#ifndef SBX_STAKEINPUT_H
#define SBX_STAKEINPUT_H
...
public:
    CSbxStake(const CTxOut& _from, const COutPoint& _outPointFrom, const CBlockIndex* _pindexFrom) :
            CStakeInput(_pindexFrom), outputFrom(_from), outpointFrom(_outPointFrom) {}

    static CSbxStake* NewSbxStake(const CTxIn& txin);
    ...
    bool IsZSBX() const override { return false; }
    bool ContextCheck(int nHeight, uint32_t nTime) override;
};


#endif //SBX_STAKEINPUT_H
