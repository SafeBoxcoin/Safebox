...
bool CLegacyZSbxStake::InitFromTxIn(const CTxIn& txin)
{
    // Construct the stakeinput object
    if (!txin.IsZerocoinSpend())
        return error("%s: unable to initialize CLegacyZSbxStake from non zc-spend", __func__);
    ...
    *this = CLegacyZSbxStake(spend);

    // Find the pindex with the accumulator checksum
    if (!GetIndexFrom())
        return error("%s : Failed to find the block index for zsafebox stake origin", __func__);

    // All good
    return true;
}

CLegacyZSbxStake::CLegacyZSbxStake(const libzerocoin::CoinSpend& spend) : CStakeInput(nullptr)
{
...
const CBlockIndex* CLegacyZSbxStake::GetIndexFrom() const
{
...
CAmount CLegacyZSbxStake::GetValue() const
{
    return denom * COIN;
}

CDataStream CLegacyZSbxStake::GetUniqueness() const
{
...
bool CLegacyZSbxStake::ContextCheck(int nHeight, uint32_t nTime)
{
    const Consensus::Params& consensus = Params().GetConsensus();
    if (!consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC_V2) || nHeight >= consensus.height_last_ZC_AccumCheckpoint)
        return error("%s : zSBX stake block: height %d outside range", __func__, nHeight);
