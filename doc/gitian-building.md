...
*Setup instructions for a gitian build of SafeBox Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the SBX
...
to the SBX GitHub release page.
...
- [Building SafeBox Core](#building-safebox-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/safebox-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/safebox-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/safebox-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/safebox-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/safebox-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/safebox-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/SafeBoxcoin/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@safebox.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/safebox-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/safebox-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/safebox-osx-*-build.assert
```
