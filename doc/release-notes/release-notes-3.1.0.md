SafeBox Core version *3.1.0* is now available from:  <https://github.com/SafeBoxcoin/Safebox/releases>
...
SafeBox Core v3.1.0 is a mandatory update for all users. This release contains new consensus rules and improvements that are not backwards compatible with older versions. Users will have a grace period of one week to update their clients before enforcement of this update is enabled.
...
zSBX Staking is here! zSBX staking will be activated on the morning of the 8th of May 2018. With the release of zSBX staking, there are effectively 2 versions of zSBX, zSBX minted on the 3.0.6 SBX wallet or lower, and zSBX minted on SBX wallet version or higher. New features in this release will require the use of zSBX v2, zSBX minted on this wallet release 3.1.0 or later. If you currently hold zSBX v1 and wish to take advantage of zSBX staking and deterministic zSBX, you will need to spend the zSBX v1 to yourself and remint zSBX v2.
Note: To find your zSBX version, click the privacy tab, then the zSBX Control button then expand the arrows next to the desired denomination.


### Deterministic zSBX Seed Keys

zSBX is now associated with a deterministic seed key. With this seed key, users are able to securely backup their zSBX outside of the wallet that the zSBX had been minted on. zSBX can also be transferred from wallet to wallet without the need of transferring the wallet data file.


### Updated zSBX minting

zSBX minting now only requires 1 further mint (down from 2) to mature. zSBX mints still require 20 confirmations.  Mints also require that the 'second' mint is at least two checkpoints deep in the chain (this was already the case, but the logic was not as precise).


### zSBX Search

Users will now have the ability to search the blockchain for a specific serial # to see if a zSBX denomination has been spent or not.



SBX/zSBX Staking and Masternode Rewards
--------------

### SBX, zSBX and Masternode Payment Schedule

To encourage the use of zSBX and increase the SBX zerocoin anonymity set, the SBX payment schedule has been changed to the following:

If a user staking zSBX wins the reward for their block, the following zSBX reward will be: 
- 3 zSBX (3 x 1 denominations) rewarded to the staker, 2 SBX rewarded to the masternode owner and 1 SBX available for the budget. This is a total block reward of 6 SBX, up from 5.

If a user staking SBX wins the reward, the following amounts will be rewarded: 
- 2 SBX to the SBX staker, 3 SBX to the Masternode owner and 1 SBX available for the budget. This is a total block reward of 6 SBX, up from 5.


### Return change to sender when minting zSBX

Previously, zSBX minting would send any change to a newly generated "change address". This has caused confusion among some users, and in some cases insufficient backups of the wallet. The wallet will now find the contributing address which contained the most SBX and return the change from a zSBX mint to that address.
...
The visual layout of the SBX Qt wallet has undergone a near-complete overhaul.
...
-	Enable and disable the auto zSBX minting feature. This is enabled by default and the enablezeromint=0 setting in the safebox.conf file will overwrite the GUI option.
-	The percentage of autominted zSBX can now be set from 1 to 100, changed from 10 – 100.
-	The stake split threshold can now be set VIA the wallet options. This setting is an advanced feature for those wishing to remain staking regular SBX.
...
### Summary
 ...
 The SBX wallet can now have user selected directories for automatic backups of the wallet data file (wallet.dat). This can be set by adding the following lines to the safebox.conf file, found in the SBX data directory.
...
* If zSBX backup, auto generated name is `wallet-autozpivbackup.dat-<year>-<month>-<day>-<hour>-<minute>-<second>`
...
SBX Daemon & Client (RPC Changes)
--------------

### RPC Ban Management

The SBX client peer bans now have additional RPC commands to manage peers. Peers can be banned and unbanned at will without the need to restart the wallet client. No changes have been made to the conditions resulting in automatic peer bans. New RPC commands: `setban`, `listbanned`, `clearbanned`, and `disconnectnode`
...
This allows running safeboxd without having to do any manual configuration.
...
Search the zerocoin database for a zerocoinspend transaction that contains the given serial. This will be a helpful tool for the SBX support group, which often times sees users say "I didn't spend that zSBX". This RPC call allows for support to grab the serial, and then find the spend tx on the chain.
...
As voted on by the SBX masternodes, the monthly budget available to be utilised has been increased to 42,000 SBX / month. This SBX only has the opportunity to be raised once per month (paid to winning proposals) with any unused SBX not created by the blockchain.

### Budget Finalization Fee

The SBX finalization fee for successful proposals has now been reduced, this fee is now 5 SBX down from 50 SBX. The total fee outlay for a successful proposal is now a total of 55 SBX.
...
-	Enforced v1 zSBX spends to require a security level of 100
-	Updates to zSBX spends to avoid segfaults
-	Updates to configuration will now reflect on the privacy tab
-	Fixed a  bug that would not start masternodes from the SBX-Qt masternodes tab
-	Updated SBX-Qt tooltips
...
Several critical security flaws in the zerocoin protocol and SBX's zerocoin implementation have been patched. Enough has changed that new zerocoins are distinct from old zerocoins, and have been labelled as *version 2*. When using the zSBX Control dialog in the QT wallet, a user is able to see zSBX marked as version 1 or 2.

zPoS (zSBX staking)
--------------

Once a zSBX has over 200 confirmations it becomes available to stake. Staking zSBX will consume the exact zerocoin that is staked and replace it with a freshly minted zerocoin of the same denomination as well as a reward of three 1 denomination zSBX. So for example if a 1,000 zSBX denomination is staked, the protocol replaces that with a fresh 1,000 denomination and three1 denomination zSBXs.

Secure Spending
--------------

Version 1 zerocoins, as implemented by [Miers et. al](http://zerocoin.org/media/pdf/ZerocoinOakland.pdf), allow for something we describe as *serial trolling*. Spending zerocoins requires that the spender reveal their serial number associated with the zerocoin, and in turn that serial number is used to check for double spending. There is a fringe situation (which is very unlikely to happen within SBX's zerocoin implementation due to delayed coin accumulation) where the spender sends the spending transaction, but the transaction does not immediately make it into the blockchain and remains in the mempool for a long enough duration that a *troll* has enough time to see the spender's serial number, mint a new zerocoin with the same serial number, and spend the new zerocoin before the original spender's transaction becomes confirmed. If the timing of this fringe situation worked, then the original spender's coin would be seen as invalid because the troll was able to have the serial recorded into the blockchain first, thus making the original spender's serial appear as a double spend.
...
Zerocoins, or zSBX, are now deterministically generated using a unique 256 bit seed. Each wallet will generate a new seed on its first run. The deterministic seed is used to generate a string of zSBX that can be recalculated at any time using the seed. Deterministic zSBX allows for users to backup all of their future zSBX by simply recording their seed and keeping it in a safe place (similar to backing up a private key for SBX). The zSBX seed needs to remain in the wallet in order to spend the zSBX after it is generated, if the seed is changed then the coins will not be spendable because the wallet will not have the ability to regenerate all of the private zSBX data from the seed. It is important that users record & backup their seed after their first run of the wallet. If the wallet is locked during the first run, then the seed will be generated the first time the wallet is unlocked.

Zerocoin Modulus
--------------

SBX's zerocoin implementation used the same code from the ZCoin project to import the modulus use for the zerocoin protocol. The chosen modulus is the 2048 bit RSA number created for the RSA factoring challenge. The ZCoin project's implementation (which SBX used) improperly imported the modulus into the code. This flaw was discovered by user GOAT from the [Civitas Project](https://github.com/eastcoastcrypto/Civitas/), and reported to SBX using the bug bounty program. The modulus is now correctly imported and SBX's accumulators have been changed to use the new proper modulus.
...
### Core Features
 - #582 `cb1632520` [Core] zSBX v2: zPoS dzSBX ezSBX external backup and budget fixes (too many to list)
...
### GUI
 - #572 `d9b23fe60` [Qt] Refresh zSBX balance after resetting mints or spends (warrows)
 ...
 - #506 `ae72bf4e2` [Qt] Autoscroll to end of zSBX status output (Mrs-X)
...
### RPC/REST
 ...
 - #543 `e4522ff07` [RPC] Segfault safebox-cli getinfo while loading block index (Mrs-X)
...
### Wallet
 - #570 `8c8350b59` [Wallet] Add a check on zSBX spend to avoid a segfault (warrows)
 ...
 - #448 `222ef6e6b` [Wallet] Return change to sender when minting zSBX. (presstab)
 - #445 `fc570fc1e` [Wallet] Only require 1 mint to be added before spending zSBX. (presstab)
