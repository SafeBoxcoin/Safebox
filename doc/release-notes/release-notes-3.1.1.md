SafeBox Core version *3.1.1* is now available from:  <https://github.com/SafeBoxcoin/Safebox/releases>
...
SafeBox Core v3.1.1 is a non-mandatory update to address bugs and introduce minor enhancements that do not require a network change.
...
### Fix spending for v1 zSBX created before block 1050020

The transition to v2 zSBX and reset of the accumulators caused blocks 1050000 - 1050010 to be accumulated twice. This was causing a number v1 zSBX to not create valid witnesses, and thus were not spendable. This problem is fixed by double accumulating blocks 1050000-1050010 when creating the witness. Any user that had issues spending zSBX v1 will now be able to convert that into SBX and then zSBX v2 (if desired).

### Adjustment to staking properties to reduce orphaned blocks

zSBX stake set to update more frequently and lowering the stake hashdrift to 30 seconds to reduce the number of orphans being experienced by SBX stakers.
...
### Core Features
 ...
 - #609 `6b73598b9` [MoveOnly] Remove zSBX code from main.cpp (presstab)
...
### GUI
 ...
 - #600 `217433561` [GUI] Only enable/disable PrivacyDialog zSBX elements if needed. (presstab)
...
### Wallet
 ...
 - #603 `779d8d597` Fix spending for v1 zSBX created before block 1050020. (presstab)
