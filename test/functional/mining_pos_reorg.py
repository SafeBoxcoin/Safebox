...
class ReorgStakeTest(SbxTestFramework):
    ...
    def check_money_supply(self, expected_safebox):
        # verify that nodes have the expected SBX supply
        safebox_supply = [self.nodes[i].getsupplyinfo(True)['transparentsupply']
                      for i in range(self.num_nodes)]
        assert_equal(safebox_supply, [DecimalAmt(expected_safebox)] * self.num_nodes)
    ...
    def run_test(self):
        ...
        # Verify that SBX supply was properly updated after the reorgs
        self.log.info("Check SBX supply...")
