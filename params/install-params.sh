...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/SBXParams"
    else
        PARAMS_DIR="$HOME/.safebox-params"
    fi
fi
...
function install_params {
    ...
    if ! [ -f "$filename" ]
    then
        filename="share/safebox/$filename"
    fi
...
function main() {
    ...
    cat <<EOF
SBX - install-params.sh

This script will install the SBX zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common SBX zkSNARK parameters. Note that it is
